module.exports = app => {
  ...
  //  导入新闻数据
   router.get('/news/init', async (req, res) => {
    ...
    const newsList = newsTitles.map(title => {
      const randomCats = cats.slice(0).sort((a, b) => Math.random() - 0.5)
      return {
        articleCate: randomCats.slice(0, 2),
        title: title
      }
    })
  ...
  router.get('/news/list', async( req, res) => {
    //通过查询一个顶级父类，再通过populate关联查询出它的子信息
    // const parent = await Category.findOne({
    //   name:'新闻资讯'
    // }).populate({
    //   path:'children',
    ...
    //mongoose聚合查询,通过条查询语句查询出信息
    const cats = await Category.aggregate([
      { $match: { parents: parent._id } }, //parents为需要查询字段，
      {
        $lookup: {
          from:'articles', //为关联的数据模型，需要写成模型名的小写复数形式
          localField:'_id',
    ...
    cats.unshift({
      ...
      newsList: await Article.find().where({
        articleCate: { $in: subCats }
      }).populate('articleCate').limit(5).lean()
    })

    cats.map( cat => {
      cat.newsList.map(news => {
        news.categoryName = (cat.name === '热门')
        ? news.articleCate[0].name : cat.name
        return news
      })
      return cat
    })
  ...
  router.get('/heroes/list', async( req, res) => {
    ...
    const cats = await Category.aggregate([
      ...
      {
        $lookup: {
          from:'heroes', //为关联的数据模型，需要写成模型名的小写复数形式
          localField:'_id',
